...
class PayNotifyCallBack extends WxPayNotify
{
    ...
    public function NotifyProcess($data, &$msg)
    {
        ...
        //写代码
        file_put_contents('./Runtime/Logs/pay/wxlog.txt',$result['openid'].'--'.time()."\n",FILE_APPEND);
        ...
        else
        {
            ...
            if (round($total_fee + $firstRow['account_money'], 2) < round($firstRow['money'], 2)) {
                ...
                if (!$purchase->execute($sql)) {
                    file_put_contents('./Runtime/Logs/pay/wxlog.txt',$trade_no.'--'.$out_trade_no.'-把支付宝支付的钱加在account_money里失败-'.$time."\n",FILE_APPEND);
                    $this->SetReturn_code("SUCCESS");
                    $this->SetReturn_msg("OK");
                    return;
                }
            ...
            $purchase->startTrans();
            $sql = "insert into ecs_temp_payment (temp_purchase_sn,time,from_user,to_user,from_account,to_account,method,type,user_id,money,client_from) values ('$out_trade_no',".time().",'{$firstRow['temp_buyers_mobile']}','品材网支付','$open_id','hbz@pcw268.com',0,0,'{$firstRow['suppliers_id']}','$total_fee',1)";
