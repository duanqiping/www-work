...
<body>
    ...
    <div class="tools auth-botton">
        <a id="add-group" class="btn" href="<?php echo U('createRule?group=wechat');?>">新 增</a>
        <a url="<?php echo U('changestatus?method=resumerule');?>" class="btn ajax-post" target-form="ids" >启 用</a>
        <a url="<?php echo U('changestatus?method=forbidrule');?>" class="btn ajax-post" target-form="ids" >禁 用</a>
        <a url="<?php echo U('changestatus?method=deleterule');?>" class="btn ajax-post confirm" target-form="ids" >删 除</a>
    </div>
	<!-- 数据列表 -->
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<td><a href="<?php echo U('Rule/editRule?id='.$vo['id']);?>"><?php echo ($vo["name"]); ?></a> </td>
            <td><?php echo ($vo["title"]); ?></td>

            <td><?php echo ($vo["status_text"]); ?></td>

			<td><?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('Rule/changeStatus?method=forbidrule&id='.$vo['id']);?>" class="ajax-get">禁用</a>
				<?php else: ?>
				<a href="<?php echo U('Rule/changeStatus?method=resumerule&id='.$vo['id']);?>" class="ajax-get">启用</a><?php endif; ?>
				<a href="<?php echo U('Rule/changeStatus?method=deleterule&id='.$vo['id']);?>" class="confirm ajax-get">删除</a>
                </td>
...
<script type="text/javascript" charset="utf-8">
    //导航高亮
    highlight_subnav('<?php echo U('Rule/wechat');?>');
</script>
