...
//$openId = $_GET["openId"];
$order_id = $_GET['temp_purchase_id'];
//$openId =  $_GET['openId'];

$conn =  mysql_connect('localhost', 'pcw','k+b/lLv4K8D2ZUg1');
if(!$conn){
    exit('数据库连接失败');
}
$sql = 'set names utf8';
...
//查询订单信息
$sql_order = "select temp_purchase_id,temp_purchase_sn,buyers_id,time,money,name,address,mobile,state,method,description,receive_time,finish_time,method,transportation,account_money,payer_id from b2b_pcy_purchase WHERE state=1 AND is_delete=0 AND temp_purchase_id='$order_id'";
$rs_order = mysql_query($sql_order);
$res_order = mysql_fetch_assoc($rs_order);

//查看红包比例
$time = time();
$sql = "select cash_back from b2b_pcy_temp_bonus WHERE bonus_id=1 AND use_end_date>'$time'";
...
$total_fee = intval(round($res_order['money']*(1+$cash_back),2)*100);
...
$input->SetNotify_url("http://www.pcw365.com/ecshop2/MobileAPI/weixinpay/order/app/notifyB2Bother.php");
