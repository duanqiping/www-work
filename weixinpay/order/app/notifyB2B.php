    ...
    public function NotifyProcess($data, &$msg)
    {
        ...
        if ($pos !== false) {
            ...
            if ($firstRow['buyers_id'] == $payer_id) /** 自己付的款(非代付), 把payer_id置为1*/ {
                $sql = "update b2b_pcy_purchase set method = 6,state = 2,actually_money = actually_money + '$total_fee',difference_money=difference_money - '$total_fee',pay_time=" . time() . ",payer_id='-1' where temp_purchase_sn = '$out_trade_no'";
            } else {
                $sql = "update b2b_pcy_purchase set method = 6,state = 2,actually_money = actually_money + '$total_fee',difference_money=difference_money - '$total_fee',pay_time=" . time() . " where temp_purchase_sn = '$out_trade_no'";
            }
