/**
 * @api {post} v3.1/myecshop2/home/clever/handle 生成材料清单
 * @apiVersion 3.1.1

 * @apiGroup clever
 * @apiParam {String} Classfy 施工类型
 * @apiParam {String} Area 总面积
 * @apiParam {String} RoomN 室数量
 * @apiParam {String} SittingN 厅数量
 * @apiParam {String} KitchenN 厨房数
 * @apiParam {String} ToiletN 卫生间数
 * @apiParam {String} BalconyN 阳台数
 * @apiParam {String} DeskType 柜台样式
 * @apiParam {String} DeskHight 柜台高
 * @apiParam {String} SittingArea 厅面积
...
/**
 * @api {post} v3.1/myecshop2/home/clever/add 批量加入购物车
 * @apiVersion 3.1.1

 * @apiGroup clever
 * @apiParam {object} goods_arr 商品对象
 * @apiParam {String} goods_arr.goods_id 商品id号
 * @apiParam {String} goods_arr.amount 商品数量
 * @apiParam {String} type 1辅材 2主材
 * @apiParam {String} area_id 城市id号

 * @apiSuccessExample {json} 成功返回结果-
 *{"success":"true","data":"成功加入购物车"}

 * @apiErrorExample {json} 失败返回结果：
 *{"success":"false","error":{"msg":"加入购物车失败","code":"4921"}}
 **/
 ...
 **/
    ...
    /**
     * @api {post} v3.1/myecshop2/center/push/some 消息推送到指定用户（指定用户是指下过单的老用户，注意该接口不要随意调用）
     * @apiVersion 3.1.1

     * @apiGroup push
     * @apiParam {String} title 消息标题
     * @apiParam {String} content 消息内容
     * @apiParam {String} user_type 1老用户 2新用户

     * @apiSuccessExample {json} 成功返回结果-
     *{"success":"true","data":{"msg":"推送成功"}}
     *
     *  @apiErrorExample {json} 失败返回结果：
     *{"success":"false","error":{"msg":"ios用户推送消息入库失败","code":"4800"}}
     *{"success":"false","error":{"msg":"安卓用户推送消息入库失败","code":"4800"}}
     *{"success":"false","error":{"msg":"title和content不能为空","code":"4800"}}
     **/

    /**
     * @api {post} v3.1/myecshop2/center/push/push_all 消息推送到全部用户（注意该接口不要随意调用）
     * @apiVersion 3.1.1
    ...
    /**
     * @api {post} v3.1/myecshop2/center/push/one 消息推送到个人（注意该接口不要随意调用）
     * @apiVersion 3.1.1
     ...
     *
     *  @apiErrorExample {json} 失败返回结果：
     *{"success":"false","error":{"msg":"用户没有绑定推送","code":"4804"}}
     *{"success":"false","error":{"msg":"title和content不能为空","code":"4800"}}
 ...
 * @apiParam {String} [temp_purchase_id] 订单id号,补货单需要
 ...
 **/
    ...
    /**
     * @api {post} v3.1/myecshop2/home/order/clickAdd 点击补货 或取消补货 按钮
     * @apiVersion 3.1.1

     * @apiGroup Order
     * @apiParam {String} temp_purchase_id  订单ID号
     * @apiParam {String} act  按钮类型 add进行补货 cancel取消补货
     *
     * @apiSuccessExample {json} 成功返回结果:
 ...
 *
 * @apiErrorExample {json} 失败返回结果：
   * 如果有补货订单，data 将返回要补货订单的详细信息（数据可以参考订单详情）
